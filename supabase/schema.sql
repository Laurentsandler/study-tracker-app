...
-- =============================================
-- 9. DISMISSED SHARED ASSIGNMENTS
-- =============================================
-- Track which shared assignments a user has dismissed (hidden from their view)
...
-- Indexes for dismissed shared assignments
CREATE INDEX IF NOT EXISTS idx_user_dismissed_shared_assignments_user ON user_dismissed_shared_assignments(user_id);
CREATE INDEX IF NOT EXISTS idx_user_dismissed_shared_assignments_shared ON user_dismissed_shared_assignments(shared_assignment_id);

-- Enable RLS on dismissed shared assignments
ALTER TABLE user_dismissed_shared_assignments ENABLE ROW LEVEL SECURITY;

-- Dismissed Shared Assignments Policies
DROP POLICY IF EXISTS "Users can view their dismissed assignments" ON user_dismissed_shared_assignments;
CREATE POLICY "Users can view their dismissed assignments" ON user_dismissed_shared_assignments
    FOR SELECT USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can dismiss assignments" ON user_dismissed_shared_assignments;
CREATE POLICY "Users can dismiss assignments" ON user_dismissed_shared_assignments
    FOR INSERT WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can undismiss their assignments" ON user_dismissed_shared_assignments;
CREATE POLICY "Users can undismiss their assignments" ON user_dismissed_shared_assignments
    FOR DELETE USING (user_id = auth.uid());

-- =============================================
-- 10. EDX COURSE PROGRESS TRACKING
-- =============================================
...
-- edX Progress Logs (individual study sessions)
